...
Key design principles:
  1. SERVER-SIDE SHORTCUTS run BEFORE LLM — no wasted API call for simple yes/no
  2. confirmCreateShop is OVERRIDDEN — always becomes createShop silently
  3. DIRECT PATH — if first transcript has shop + products clear, skip to previewOrder
  4. Salesman never sees shop creation — it happens silently in background
  5. Confirmation UI shows shop name + all items from previewOrder
"""
...
async def agent_node(state: VoiceOrderState) -> dict:
    ...
    if current_step == "awaiting_confirmation" and transcript:
        ...
        if _matches_confirm(transcript):
            ...
            logger.info(f"SHORTCUT A: awaiting_confirmation + yes → confirmOrder | session={session_id[:8]}")
            ...
            logger.info(f"SHORTCUT A: awaiting_confirmation + no → cancelConversation | session={session_id[:8]}")

    # ── Shortcut B: awaiting_shop_creation_confirm — auto-create (no asking) ──
    # With new prompt, LLM calls createShop directly. But if it somehow still
    # calls confirmCreateShop, we handle the salesman's yes/no here.
        ...
        if _matches_confirm(transcript):
            ...
            shortcut_args = {
                ...
                "message_en":    f"Registering '{pending_shop_name}'. What would you like to order?",
                "message_local": f"'{pending_shop_name}' register ho rahi hai. Kya order dena hai?",
            }
            logger.info(f"SHORTCUT B: shop_creation_confirm + yes → createShop '{pending_shop_name}' | session={session_id[:8]}")
        elif _matches_cancel(transcript):
            shortcut_tool = "respondMessage"
            shortcut_args = {
                "message_en":    "Okay, which shop would you like to place an order for?",
                "message_local": "Theek hai, kaunsi dukaan ka order dena hai?",
            }
            logger.info(f"SHORTCUT B: shop_creation_confirm + no → ask again | session={session_id[:8]}")

    # ── Shortcut C: clarify_count >= 3 — force resolution ─────────────────────
    if shortcut_tool is None and clarify_count >= 3:
        clarif      = memory.get("clarification_state") or {}
        clarif_type = clarif.get("type", "")
        if clarif_type in ("confirmCreateShop",):
            pending_shop_name = clarif.get("shop_name", "Naya Store")
            shortcut_tool = "createShop"
            shortcut_args = {
                "shop_name":     pending_shop_name,
                "message_en":    f"Creating '{pending_shop_name}'. What products do you need?",
                "message_local": f"'{pending_shop_name}' bana raha hoon. Kya order dena hai?",
            }
            logger.info(f"SHORTCUT C: clarify_count={clarify_count}>=3, forcing createShop | session={session_id[:8]}")
        elif clarif_type == "chooseShop":
            matches = clarif.get("matches", [])
            if matches:
                best = matches[0]
                memory["selected_shop"] = {
                    "shop_id":   best.get("shop_id", ""),
                ...
                logger.info(f"SHORTCUT C: clarify_count>=3, auto-selecting first shop | session={session_id[:8]}")
    ...
    # ── SERVER-SIDE OVERRIDE: confirmCreateShop → createShop (silent) ─────────
    # Even if the LLM calls confirmCreateShop (ignoring our prompt instructions),
    # we intercept and execute createShop directly. Salesman never sees a
    # "should I create this shop?" question — it just happens silently.
    if tool == "confirmCreateShop":
        shop_name_from_args = args.get("shop_name", "")
        logger.info(f"OVERRIDE: confirmCreateShop → createShop '{shop_name_from_args}' | session={session_id[:8]}")
        tool = "createShop"
        args = {
            "shop_name":     shop_name_from_args,
            "message_en":    f"Registering '{shop_name_from_args}'. What would you like to order?",
            "message_local": f"'{shop_name_from_args}' register ho rahi hai. Kya order dena hai?",
        }
    ...
    # ── confirmCreateShop — should never reach here (overridden above) ─────────
    elif tool == "confirmCreateShop":
        # Fallback — already overridden, but just in case
        memory_updates["clarification_state"] = {
